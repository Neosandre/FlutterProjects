...
class _FeedScreenState extends State<FeedScreen> {
   ...
   getFollowing() async {
     QuerySnapshot snapshot = await FirebaseFirestore.instance.collection('following').doc(currentUserId).collection('userFollowing').get();
     snapshot.docs.forEach((doc) {
       //String item = json.decode(json.encode(doc['ownerId']));
       //followingUserList.add(doc.data().toString());

       //UserDetails userDetails = new UserDetails.fromMap(doc);
setState(() {
       ...
       child: Center(
         child: Column(
           children: [
             ...
             Text(
               'Not posts,follow users to see their posts...\n',
               style: TextStyle(
                   fontSize: 30,
                   color: Colors.grey,
      ...
      actions: [
        ...
        Padding(
           ...
           child: GestureDetector(
             onTap: ()=>Navigator.push(context, MaterialPageRoute(builder: (ctx)=>ChatList(widget.user))),
             child:Icon(Icons.chat,
           color:selectedChat == false ? Colors.purple :Colors.white ,
                        ...
                        switch (postDocs[index]['type']) {
                          case 'pp':
                            ...
                            return GestureDetector(
                              ...
                              onTap: ()=>Navigator.push(context, MaterialPageRoute(builder: (context)=>PostViewPoll(post,getTimeDifferenceFromNow(dateTime),converted,postDocs[index]["likedBy"],postDocs[index]['commentCount'],widget.user))),
                          ...
                          case 'pq':
                            ...
                            return GestureDetector(
                              ...
                              onTap: ()=>Navigator.push(context, MaterialPageRoute(builder: (context)=>PostViewQuiz(widget.user, postQuiz,getTimeDifferenceFromNow(dateTime),converted,postDocs[index]['userWhoAnswered'],postDocs[index]["likedBy"],postDocs[index]["commentCount"]))),
...
class chatselected extends ChangeNotifier {
