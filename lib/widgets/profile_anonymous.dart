...
import '../screens/profilepostviews/pollview.dart';

class ProfileAnonymous extends StatelessWidget {
  UserModel userModel;
  ProfileAnonymous(this.userModel);
 ...
 // @override
 // State<ProfileAnonymous> createState() => _ProfileAnonymousState();
//}

//class _ProfileAnonymousState extends State<ProfileAnonymous> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream: FirebaseFirestore.instance
          .collection('post')
          .where('name', isEqualTo: "anonymous").orderBy('time',descending: true)
          .snapshots(),
      builder: (context,AsyncSnapshot<QuerySnapshot> snapshot) {
        if (snapshot.hasData) {
          final postDocs = snapshot.data!.docs;
          return ListView.builder(
            key: PageStorageKey<String>('anonymous'),
            ...
            itemBuilder: (context, index) {
              switch (postDocs[index]['type']) {
                case 'pp':
                  ...
                  return GestureDetector(
                    onLongPress: currentUserId != "jLhQUkYfk2Nelc3H9aRn45FJpap2"? ()=>report(context, currentUserName, postDocs[index]['postId'], post.name, post.userId,post.text,post.postImage):()=>deletePost(postDocs[index]['postId'],context),

                    onTap: ()=>Navigator.push(context, MaterialPageRoute(builder: (context)=>PollView(post,getTimeDifferenceFromNow(dateTime),converted,postDocs[index]["likedBy"],postDocs[index]['commentCount'],userModel))),

                    child: Card(
                        color: Colors.black,
                        shadowColor: Colors.purple,
                        elevation: 30,
                        child: PostPoll(post, formattedTime, converted,
                            postDocs[index]["likedBy"],
                            postDocs[index]["commentCount"], userModel)
                    ),
                  );
               ...
               case 'pq':
                ...
                return GestureDetector(
                  onLongPress:  currentUserId == postDocs[index]['userId'] || currentUserId == "jLhQUkYfk2Nelc3H9aRn45FJpap2"
                      ? ()=>deletePost(postDocs[index]['postId'],context):()=>report(context, currentUserName, postDocs[index]['postId'], postQuiz.name, postQuiz.userId,postQuiz.text,''),

                  onTap: ()=>Navigator.push(context, MaterialPageRoute(builder: (context)=>QuizView(userModel, postQuiz,getTimeDifferenceFromNow(dateTime),converted,postDocs[index]['userWhoAnswered'],postDocs[index]["likedBy"],postDocs[index]["commentCount"]))),

                  child: Card(
                      color: Colors.black,
                      shadowColor: Colors.purple ,
                      elevation: 30,
                      child: PostQuiz(userModel, postQuiz,formattedTime,converted,answerList,postDocs[index]['userWhoAnswered'],postDocs[index]["likedBy"],postDocs[index]["commentCount"] )),
                );
              ...
              case 'pg':
                ...
                return GestureDetector(
                  onLongPress: currentUserId == postDocs[index]['userId'] || currentUserId == "jLhQUkYfk2Nelc3H9aRn45FJpap2"
                      ?()=>deletePost(postDocs[index]['postId'],context): ()=>report(context, currentUserName, postDocs[index]['postId'], postGuess.name, postGuess.userId,postGuess.text,''),

                  onTap: ()=>Navigator.push(context, MaterialPageRoute(builder: (context)=>PostViewGuess(userModel,postGuess,getTimeDifferenceFromNow(dateTime),expireTime,postDocs[index]["likedBy"],postDocs[index]['commentCount']))),

                  child: Card(
                      color: Colors.black,
                      shadowColor: Colors.purple,
                      elevation: 30,
                      child:PostGuess(userModel,postGuess,formattedTime,expireTime,postDocs[index]["likedBy"],postDocs[index]['commentCount'])),
                );
              }
              return Container();
            },
