...
class BottomLayout extends StatefulWidget {
  ...
  final String chatId;
  //final VoidCallback onSendMessageClick;
  //const BottomLayout({Key? key}) : super(key: key);
  const BottomLayout(/*this.focusNode*/ this.replyMessage, this.onCancelReply,
      this.userModel, this.chatId,/*this.onSendMessageClick*/);
...
class _BottomLayoutState extends State<BottomLayout> {
  ...
  print(widget.userModel.id);
   ...
   sendPushNotification(name, msg) async {
  ...
  String getRandomString(int length) => String.fromCharCodes(Iterable.generate(
      length,
      (_) => BottomLayout._chars
          .codeUnitAt(_rnd.nextInt(BottomLayout._chars.length))));
  ...
  _sendImage() async {
    ...
    if (_pickedImage != null) {
      ...
      await FirebaseFirestore.instance.collection('chats').doc(fileId).set({
        'messageId': fileId,
        'text': "image",
        'time': Timestamp.now(),
    ...
    sendPushNotification(currentUserName, 'image...');
  ...
  void _sendMessage() async {
   ...
   await sendPushNotification(currentUserName, _controller.text);
    widget.onCancelReply();
    _controller.clear();
...
emojiShowing=false;
    if (await Permission.microphone.request().isGranted) {
      ...
      Platform.isAndroid
          ? await _audioRecorder
              ...
              .then((_) {
              Future.delayed(Duration(minutes: 5)).then((_) async {
  ...
  Future<void> _stop() async {
    ...
    try {
      ...
      widget.onCancelReply();
      sendPushNotification(currentUserName, 'audio message...');
    } catch (error) {
...
//print( '${_formatNumber(_recordDuration ~/ 60)}:${_formatNumber(_recordDuration % 60)}');
  ...
  void _sendUrl() async {
    ...
    final userData = await FirebaseFirestore.instance
        ...
        .doc(currentUserId)
        .get();
    FirebaseFirestore.instance.collection('chats').doc(fileId).set({
      'messageId': fileId,
    ...
    sendPushNotification(currentUserName, 'link...');
  ...
  _sendEffects(String name)async{
    ...
    await ref
      ...
      sendPushNotification(currentUserName,'audio $name...' /*'link...'*/);
      widget.onCancelReply();
      Navigator.pop(context);
    } catch (error) {
      print('error occurred while uploading to firebase ${error.toString()}');
  ...
  final focusNodelocal = FocusNode();
